...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // Ensure the restaurants folder exists in public storage
        Storage::disk('public')->makeDirectory('restaurants');

        // 2. Define exactly 10 Indian Restaurants with image seeds
        $restaurantsData = [
            [
                'name'          => 'Spice Symphony',
                'slug'          => 'spice-symphony',
            ...
            [
                ...
                'reviews_count' => 189,
                'settings'      => ['theme' => 'dark'],
                'image_seed'    => 'royal-biryani-durbar',
            ],
            [
                'name'          => 'Shahi Dawat',
                'slug'          => 'shahi-dawat',
            ...
            [
                ...
                'reviews_count' => 156,
                'settings'      => ['theme' => 'dark'],
                'image_seed'    => 'malabar-coast',
            ],
        ];

        // picsum.photos seed IDs for food/restaurant themed images (using consistent numeric seeds)
        $picsumSeeds = [
            'spice-symphony'     => 292,   // warm tones
            'maharaja-palace'    => 431,   // ornate interior
            'zayka-haveli'       => 674,   // warm food
            'awadhi-rasoi'       => 999,   // cozy restaurant
            'tandoori-nights'    => 137,   // fire/grill
            'royal-biryani-durbar' => 488, // biryani colours
            'shahi-dawat'        => 823,   // opulent
            'delhi-darbar'       => 340,   // city dining
            'bukhara-heritage'   => 573,   // heritage tones
            'malabar-coast'      => 217,   // coastal
        ];

        $restaurants = [];
        foreach ($restaurantsData as $data) {
            $imageSeed = $data['image_seed'];
            $picSeedId = $picsumSeeds[$imageSeed] ?? 100;
            $imagePath = null;

            // Try to download the image from picsum.photos
            try {
                $imageUrl  = "https://picsum.photos/seed/{$picSeedId}/800/500";
                $response  = Http::timeout(15)->withOptions(['verify' => false])->get($imageUrl);

                if ($response->successful()) {
                    $filename  = "restaurants/{$imageSeed}.jpg";
                    Storage::disk('public')->put($filename, $response->body());
                    $imagePath = $filename;
                    $this->command->info("✅ Downloaded cover photo for: {$data['name']}");
                } else {
                    $this->command->warn("⚠️  Failed to download image for: {$data['name']} (HTTP {$response->status()})");
                }
            } catch (\Exception $e) {
                $this->command->warn("⚠️  Could not download image for: {$data['name']} — {$e->getMessage()}");
            }
        ...
        $this->command->info('   10 Restaurants | 10 Managers | 10 Staff | 10 Customers | 50 Bookings');
        $this->command->info('   Login: admin@gmail.com / password');
        $this->command->info('   Login: manager1@gmail.com / password');
        $this->command->info('   Login: customer@gmail.com / password');
    }
}
